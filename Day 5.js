{
	...
	const instructions = [...input[1].matchAll(/move (?<much>\d+) from (?<from>\d) to (?<to>\d)/g)]

	instructions.forEach(match => {
		for (let _ = 0; _ < match.groups.much; ++_)
			data_crane9000[match.groups.to-1].push(data_crane9000[match.groups.from-1].pop())
	})
	instructions.forEach(match => {
